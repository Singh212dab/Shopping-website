...
class Component implements Component_Interface {
	...
	public function custom_block_html( $html, $data, $product ) {
			...
			if ( is_a( $product, 'WC_Product' ) ) {
				...
				if ( $attachment_ids ) {
					...
					$secondary_image_output = wp_get_attachment_image(
						...
						false,
						array(
							'class' => 'secondary-product-image attachment-shop-catalog wp-post-image wp-post-image--secondary',
		...
		if ( $product_image_hover_style !== 'none' ) {
		...
		return "<li class=\"wc-block-grid__product entry loop-entry content-bg {$action_style} {$button_style} {$boxed_class} {$hover_style}\">
				<a href=\"{$new_data->permalink}\" class=\"{$product_image_link}\">
					{$new_data->image}
